...
function Products({ showSidebar, category, products }: Props) {
  ...
  return (
    <main className="w-full px-4 mr-4 lg:mr-0">
      ...
      <nav className="gap-4 pt-8 text-gray-500 lex dark:text-gray-300 group">
        <Link
      ...
      <div className="flex flex-wrap items-center gap-4 my-8 justify-evenly md:justify-between ">
        <div className="flex flex-wrap items-stretch gap-4 ">
        <span
        className="px-4 py-4 text-xs transition-all bg-gray-100 rounded-md"
        >{products.length} products on page</span>
          {formattedTags ? (
            <>
            <span
            ...
            >Tags:
              {formattedTags?.map((tag, index) => (
                <span key={index} className="px-4 py-2 text-xs text-gray-500 bg-gray-100 rounded-md">
                  {fromSlugToName(tag)}
                </span>
              ))}
              </span>
            ...
            <span className="px-4 py-4 text-xs text-gray-500 bg-gray-100 rounded-md">No Search Tags</span>
          )}
